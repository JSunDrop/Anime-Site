...
export default function Work({ params }:{ params:{ slug:string }}){
  ...
  return (<main>
    ...
    <h3>Comments</h3>
    <div className="card">{comments.map(c => <div key={c.id} style={{marginBottom:8}}><b>{c.by?.slice?.(0,6) || 'anon'}</b> — {new Date(c.created_at).toLocaleString()}<br/>{c.text}</div>)}</div>
    <input value={text} onChange={e=>setText(e.target.value)} placeholder="Say something"/>
    <button className="btn" onClick={send} style={{marginLeft:8}}>Post</button>
  </main>)
}