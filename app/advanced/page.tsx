...
export default function Advanced(){
  ...
  return (<main>
    <h2>Advanced Search & Recommendations</h2>
    <p>Set your DOB. Server-side age RLS will only return content allowed for your age.</p>
    <input type="date" value={dob} onChange={e=>setDob(e.target.value)} style={{padding:10,borderRadius:10,border:'1px solid #1f2947',background:'#0b1224',color:'#eaf2ff'}}/>
    <div><button onClick={save} style={{marginTop:10}}>Save</button> <span style={{opacity:.8,marginLeft:10}}>{status}</span></div>
  </main>)
}